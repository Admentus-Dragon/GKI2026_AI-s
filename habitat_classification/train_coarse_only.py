...
class HabitatDataset(Dataset):
    def __init__(self):
        ...
        self.y = df["vistlendi_idx"].to_numpy().astype(np.int64)
...
model = SimpleCNN(out_dim=13).to(DEVICE)
...
for epoch in range(EPOCHS):
    ...
    val_acc = eval_epoch()
    print(f"Epoch {epoch:03d} | train_loss={total_loss:.4f} | val_coarse_acc={val_acc:.4f}")

Path("models_baselines").mkdir(exist_ok=True)
torch.save(model.state_dict(), "models_baselines/coarse_only.pt")
print("Saved models_baselines/coarse_only.pt")
