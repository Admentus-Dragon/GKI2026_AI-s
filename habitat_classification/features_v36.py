...
def extract_features(patch: np.ndarray) -> np.ndarray:
    ...
    return np.concatenate([spec_mean, spec_std, terr_mean, terr_std, terr_min, terr_max]).astype(np.float32)
...
def extract_features_batch(patches: np.ndarray) -> np.ndarray:
    ...
    feats = np.concatenate([spec_mean, spec_std, terr_mean, terr_std, terr_min, terr_max], axis=1)
